...
//   4) 최종확률 = clamp01(기본확률 + 카테고리 보너스 - 민감도 페널티)
public static class PurchaseEvaluator
{
    ...
    public static Result Evaluate(NpcProfile profile, ShopSlot slot, System.Random rng)
    {
        ...
        float finalProb = Mathf.Clamp01(baseProb + categoryBonus - sensitivityPenalty);
        r.probability = finalProb;

        bool roll = rng.NextDouble() < finalProb;
        r.willBuy = roll;
        r.reason = string.Format(
            "ratio={0:F2} base={1:F2} catBonus={2:+0.00;-0.00} sensPen={3:-0.00} → p={4:F2} roll={5}",
            ratio, baseProb, categoryBonus, sensitivityPenalty, finalProb, roll ? "BUY" : "PASS");
