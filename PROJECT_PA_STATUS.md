...
## 2026-08-25 — Visual & Player Experience Baseline

- 현재 제품 진입점은 Build Settings의 유일한 활성 씬 `Prototype_FirstDay`이다. 이 씬의 타이틀 → 온보딩 → Day 1 판매 → 감사 → 결산 → 격리 저장/복원은 D3D11 Play Mode에서 실행됐다.
- 최신 생성 월드·낮 활동·제작·상점·7일 진행은 `WorldSandbox` 전용이며 일반 타이틀에서 `UNREACHABLE`이다. `MainGame`도 비활성이다.
- 비배치 Game View에서 현재 화면 16장을 확보하고 모두 직접 검사했다. 타이틀 뒤 HUD 노출, 상단/하단 UI 경쟁, 카메라 차양 가림, 원시 큐브 채집 지점, 거대 고객 말풍선, 작은 인벤토리/결산이 핵심 UX 문제다.
- Final Presentation 레이아웃은 `MoneyHUD does not overlap objective panel` assertion에서 실패했다. 캡처가 476×1297이어서 목표 1920×1080 standalone은 확인하지 못했다.
- SaveManager v12 격리 저장은 player position을 복원했다. 생성 월드 재시작은 기존 `HARD_BLOCKER_BETA_010_PLAYER_FACING_RESTORE`가 남아 있다.
- 상세 결과: `Docs/PROJECT_STATE.md`, `Docs/VisualAudit/2026-08-25-29fb98f/VISUAL_BASELINE.md`.

## 2026-08-31 — 여름방학 진행상황 발표 스냅샷

- 2026-06-26~2026-08-31을 기준으로 기존 기능, 방학 연결 작업, 현재 도달 가능 상태와 Demo Gap을 분리한 감사 문서를 작성했다.
- 7장 발표자료와 PDF, 5분 대본, 슬라이드별 근거 문서를 생성했다. 발표용 상태 표기는 `PARTIAL`, `UNREACHABLE`, `BLOCKED`를 구분한다.
- 게임 상태는 바뀌지 않았다. `HARD_BLOCKER_BETA_010_PLAYER_FACING_RESTORE`, WorldSandbox 일반 진입 불가, 1920×1080 standalone 미확인 상태를 그대로 유지한다.
- Unity compile/PlayMode는 이번 문서 작업에서 실행하지 않아 확인 못 함이다. 코드·씬·프리팹·Packages·ProjectSettings·Save schema 변경은 없다.

## 2026-09-01 — Game Identity & Scope Audit

- 4월 원안의 관리자·역공급망 정체성과 6~7월 고정된 `resident-owner-operator`·낮/밤·마을 변화 방향을 Git/문서/코드 기준으로 대조했다.
- 현재 시스템은 생산자 4종, 전문가 4종, 성향별 소비, 유료 채용, 일정, 친밀도, 마을 변화 기반을 갖췄다. 다만 일반 Build Settings에서는 Prototype Day 1 상점 루프만 보이고 최신 M85는 `UNREACHABLE`이다.
- 캐릭터 판정은 `IDENTITY GAP`: 보리는 이름/친밀도 키만 있고 전용 프로필이 없으며, 역할 프리팹 8종은 개인 이름·약력이 비어 있고, 미라/준/아를로/타라/펠릭스 프로필은 현재 참조되지 않는다.
- 졸업 데모 정체성은 `유료 채용→생산자 매입→가공→가격→성향별 소비→정산→다음 날 마을 변화`의 3~5분 연속 인과로 권고한다.
- 감사 문서 4종만 생성했다. PPT, 코드, 씬, 프리팹, Packages, ProjectSettings, Save schema, 사용자 save는 변경하지 않았다. Unity 검증은 실행하지 않아 확인 못 함이다.
